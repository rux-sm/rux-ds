...
cell current. Newest tag `v0.1.14`. The hub, Notes and Scheduler all hold that
tag's bytes; their `PIN` files still name `v0.1.12`, because nothing vendored
has changed since it and the pin names bytes, not a position in the tag order.
...
| Task | Command | Then |
|---|---|---|
| Run | `npm run serve` → `http://localhost:8642` | open the sink, the portal, a template |
| Run every site on one origin | `npm run serve:workspace` → `http://localhost:8640` — `/` the hub, `/<app>/` each app | the switcher fills from the hub's list |
...
| `tools/` | every build and check script; `new-project.sh` starts an app or moves its pin, `roll-out.sh` moves every app, `serve.mjs` serves |
...
| `reference/` · `adoption-audit.md` | material about agent tooling, not about the kit; nothing reads it |
