...
started** — `templates/app-shell.html` is the frame, with `table-page.html` and
`form-page.html` built on it. Until the rest land the kitchen sink is still the worked
...
is preparation. Three of six exist — `app-shell.html`, `table-page.html` and
`form-page.html`; detail, empty state and error state are open. Every gate that reads
