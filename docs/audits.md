...
| 14 | `sink/harness.css` is loaded by `kitchen-sink.html` alone and is a declared input to no browser gate, so the sink's four readings do not age against the stylesheet positioning their specimens | **filed 2026-09-02** | Pre-existing, not introduced by finding 11's fix: no browser gate ever named it. `sharedInputs` plus the cell's page has no room for a per-page extra input, so expressing it is a registry design question rather than a missing entry |
